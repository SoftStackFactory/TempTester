...
.controller('EmpSettingsCtrl', ['$scope', 'employer', 'SSFAppCssService', 'ServerEmployersService',
  ...
  $scope.submitPassword = function(form) {
    ...
    UserService.updateUser($window.localStorage.token, $window.localStorage.userId, $scope.updateUser, 'CompanyUser/')
    .then(function(res) {
      if(res.status !== 200)
        return console.error('Cannot Update Password', res);
